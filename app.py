...
if pagina == "Home":
    with st.expander("⚙️ Impostazioni Globali e Layout"):
        ...
        with c_watch:
            watchlist_input = st.text_input("Watchlist (Simboli separati da virgola)", value=st.session_state.get("watchlist_tickers", "AAPL, MSFT, GOOGL, TSLA, NVDA"))
...
# ==========================================
# ALTRE PAGINE (POSIZIONI, RISCHIO, LOG)
# ==========================================
elif pagina == "Posizioni":
    st.markdown("### Portafoglio Attuale")
    if posizioni:
        df = pd.DataFrame([{ "Simbolo": p['symbol'], "Quantità": p['qty'], "Prezzo": f"${float(p['current_price']):.2f}", "P&L": f"{float(p['unrealized_plpc'])*100:.2f}%"} for p in posizioni])
        st.dataframe(df, use_container_width=True, hide_index=True)
    else:
        st.info("Nessuna posizione aperta.")

elif pagina == "Analisi Rischio":
    st.info("Metriche di rischio (Sharpe, Drawdown) in elaborazione...")

elif pagina == "Log & Override":
    st.markdown("### Operazioni d'Emergenza")
    if st.button("🛑 CHIUDI TUTTE LE POSIZIONI ORA", type="primary"):
        with st.spinner("Invio ordine di chiusura..."):
            if close_all_positions():
                st.success("Tutte le posizioni sono state chiuse.")
                time.sleep(1); st.rerun()
            else: st.error("Errore di connessione API.")
